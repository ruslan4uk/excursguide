...
export default {
    ...
    computed: {
        ...mapGetters('profile', ['user_data']),
        license() {
            return this.user_data.user_files || []
        }
    },
    methods: {
        ...mapMutations('profile', ['setLicense']),
        changeLicense() {
            ...
            axios.post('api/profile/upload/license', formData, {headers: {'Content-Type': 'multipart/form-data'}})
                .then(response => {
        ...
        deleteLicense(index) {
            axios.post('api/profile/upload/license/delete', {id: index})
                .then(response => {
                    this.setLicense(response.data)
                    this.timestamp = '?' + Math.floor(Date.now() / 1000)                    
                })
